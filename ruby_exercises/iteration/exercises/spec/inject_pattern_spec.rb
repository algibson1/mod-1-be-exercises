RSpec.describe 'inject pattern test' do
  ...
  it 'test 7' do
    ...
    elements.each do |element|
      sum_of_second_values += element.last
    end
  ...
  final_grade = 0
  grades.each do |category, grades|
    final_grade += (grades.sum.to_f/grades.length.to_f)
  end
  ...
  it 'test 10' do
   ...
   printable_menu = "Menu:\n"
   menu.each do |item, info|
    last_flavor = info[:flavors].pop
    gluten_status = "(gluten free)" if info[:gluten_free] == true
    gluten_status = "(non gluten free)" if info[:gluten_free] == false
    printable_menu << "- #{info[:flavors].join(", ")}, and #{last_flavor} #{item} #{gluten_status}\n"
   end
